...
const { EMAIL_ERROR } = require('../../constants/error.js')
const { dateToday } = require('../helpers/date-today.js')
...
const sendEmailController = (req, res) => {
  if (!req.user.isUnder18) return res.status(403).send('must be under 18')
  ...
  const emailRecipients = process.env.NODE_ENV !== 'production'
    ? [process.env.REPORT_EMAIL_RECIPIENT_TEST]
    : [emailRecipientOne].concat(emailRecipientTwo || [])
