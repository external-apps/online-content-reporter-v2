...
const ageConfirmationController = (req, reply) => {
  ...
  promise.then((activityDetails) => {
    if (ageCheck(activityDetails.getUserProfile().dateOfBirth)) {
       if (req.query.mobile) reply.json({ test: true }); reply.end()
       if(req.query.mobile){
         reply.sendFile('index.html?isUnder18=true', {root: __dirname+'../../dist/'})
       }
      reply.sendFile('index.html?isUnder18=true', {root: __dirname+'../../dist/'})
    //  reply.json({isUnder18: true})
      ...
      if(req.query.mobile){
        reply.sendFile('index.html?isUnder18=false', {root: path.join(__dirname+'../../dist/')})
      }
      reply.sendFile('index.html?isUnder18=true', {root: path.join(__dirname+'../../dist/')})
