...
const ageConfirmationController = (req, reply) => {
  ...
  promise.then((activityDetails) => {
    if (ageCheck(activityDetails.getUserProfile().dateOfBirth)) {
      console.log(req.query.token.split('?')[1].split('=')[1])//.token.split('?')[1].split('=')[1])
       if(req.query.token.split('?')[1].split('=')[1]){
         reply.json({isUnder18: true})
         reply.end()
       }
       console.log('I never reach that point!!!')
      reply.cookie('isUnder18', 'true', {  expires: new Date(Date.now() + 900000), httpOnly: false,path: '/'})
      ...
      reply.cookie('isUnder18', 'false', {  expires: new Date(Date.now() + 900000), httpOnly: false,path: '/'})
      reply.redirect('/')
    //  reply.json({isUnder18: false})
      reply.end()
    }
